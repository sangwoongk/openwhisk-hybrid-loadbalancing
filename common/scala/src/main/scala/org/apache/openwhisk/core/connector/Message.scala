...
// yanqi, add cpu limit & cpu utilization
case class ActivationMessage(override val transid: TransactionId,
                             ...
                             var cpuLimit: Double = -1.0, 
                             var cpuUtil: Double = -1.0)
    extends Message {
...
object ActivationMessage extends DefaultJsonProtocol {
  ...
  implicit val serdes = jsonFormat13(ActivationMessage.apply) // yanqi, add cpuLimit & cpuUtil
}
 ...
 */
// yanqi, add cpu util to CompletionMessage
...
object CompletionMessage extends DefaultJsonProtocol {
  ...
  implicit val serdes = jsonFormat5(CompletionMessage.apply)  // yanqi
}
...
// hermod, add warm container info to PingMessage
case class PingMessage(instance: InvokerInstanceId, cpu: Double, memory: Long, warms: Map[String, Int]) extends Message {
...
// yanqi, hermod, modify jsonFormat w.r.t PingMessage
object PingMessage extends DefaultJsonProtocol {
  def parse(msg: String) = Try(serdes.read(msg.parseJson))
  implicit val serdes = jsonFormat(PingMessage.apply _, "name", "cpu", "memory", "warms")
}
// object PingMessage extends DefaultJsonProtocol {
//   def parse(msg: String) = Try(serdes.read(msg.parseJson))
